#!/usr/bin/env bash
# Sync exe from GeoFlux release to GeoFlux-release release, update releases.json
# Usage: ./scripts/sync-release.sh [version]
#   version: specific tag to sync (e.g. "v0.2.0"), or omit for latest release
set -euo pipefail
...
if [ -f "$REPO_DIR/releases.json" ]; then
  ...
  echo "  Loaded existing releases.json from repo"
else
  echo '[]' > "$RELEASES_JSON"
